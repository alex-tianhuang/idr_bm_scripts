...
use clap::Parser;
use idrbm_core::{box_dyn_in, utils::{CompoundHeaderWriter, read_fasta_to_map, read_regions, read_variants}};
    ...
    for (protein_id, protein_group) in variants {
        ...
        for &(region_id, region_group) in protein_group {
            ...
            if !region.is_in_bounds_of(whole_sequence) {
                writeln!(err_out, "region is out of bounds for protein {} (start={}, stop={}, seq.len()={}): {}", region.start(), region.stop(), whole_sequence.len(), protein_id, region_id)?;
                continue;
            }
